...
void inputPole(Point* pole) {
	printf("Enter field coordinates : ");
...
void OutPutBuf(int buf[H][W])
{
	...
	for (int i = H-1; i >= 0; i--)
	{
		for (int j = 0; j < W; j++)
		{
			...
			else if (in >= max_shot * 0.9)setColor(3, 0);
			else if (in >= max_shot * 0.7)setColor(9, 0);
			else if (in >= max_shot * 0.4)setColor(1, 0);
			else setColor(7, 0);
			char ch = 178;
			printf("%c",ch);
			setColor(7, 0);
...
void inCircly(Point* pole, int buf[H][W], WSpace* worksp, double& N, Point circ, double R, double M) {
	for (double i = 0; i < M; i++) {
		Point ranD;
		ranD.x = rand() * 1.0 / RAND_MAX * (pole[1].x - pole[0].x) + pole[0].x;
		ranD.y = rand() * 1.0 / RAND_MAX * (pole[1].y - pole[0].y) + pole[0].y;
...
void Circly(Point* pole) {
	...
	scanf("%lf", &M);
	inCircly(pole, buf, worksp, N, circ, R,M);
	printf("Area - ");
	printf("%lf\n", Area(N, pole, M));
	OutPutBuf(buf);
}
...
void Lemniscate( Point* pole) {
	...
	WSpace* worksp = new WSpace;
	int buf[H][W] = { 0 };
	printf("Enter (x^2 + y^2)^2 = 2*a^2 * (x^2 - y^2) ");
	printf("\nEnter a : ");
	scanf("%lf", &a);
	pole[1].x = 1.41 * a;
	pole[1].y = a / 2;
...
int main()
{
	...
	int command;
	Point pole[2];
	printf("Select a shape to find the area (1 - polygon; 2 - circly; 3 - lemniscate) : ");
	scanf("%i", &command);
	switch (command) {
	case 1: Polygon(pole);
		break;
